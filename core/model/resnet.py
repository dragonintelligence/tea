...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1,norm_layer=None):
        ...
        self.bn1 = nn.BatchNorm2d(planes) #norm_layer(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3,
                               stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes) #norm_layer(planes)
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                ...
                nn.BatchNorm2d(self.expansion*planes) #norm_layer(self.expansion*planes)
            )
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, norm_layer=None):
        ...
        self.bn3 = nn.BatchNorm2d(self.expansion*planes) #norm_layer(self.expansion*planes)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, norm_layer=None):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3,
                               stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(64) #norm_layer(64)
